...
export function Navigation({ currentPage, onPageChange }: NavigationProps) {
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <nav className="fixed top-0 left-0 h-full w-20 bg-[#060911]/90 backdrop-blur-md border-r border-primary/25 z-40 hidden lg:flex flex-col items-center py-8">
        <div className="mb-10">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Tooltip */}
                <div className="absolute left-full ml-4 px-3 py-2 bg-[#060911]/95 border border-primary/30 text-white text-sm font-hud tracking-wide opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-50">
                  {item.label}
                </div>

                {/* Active indicator */}
                {currentPage === item.id && (
                  <div className="absolute -right-[1px] top-1/2 transform -translate-y-1/2 w-0.5 h-8 bg-gradient-to-b from-primary to-secondary" style={{ boxShadow: '0 0 10px rgba(0,168,255,0.8)' }} />
                )}
              </button>
            )
          })}
      ...
      {/* Mobile Navigation */}
      <nav className="fixed top-0 left-0 right-0 bg-[#060911]/90 backdrop-blur-md border-b border-primary/25 z-40 lg:hidden">
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="absolute top-full left-0 right-0 bg-[#060911]/95 backdrop-blur-md border-b border-primary/25">
